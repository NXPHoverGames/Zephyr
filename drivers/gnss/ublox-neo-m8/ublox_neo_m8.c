...
#define TIMEOUT_COUNT_UBX 10000000
#define TIMEOUT_COUNT_NMEA 10000000
...
#if 0
	rc = i2c_write_read(cfg->i2c_dev, cfg->i2c_addr, &addr, sizeof(addr), buffer, 1);
#else
	rc = uart_poll_in(cfg->uart_dev, buffer);
...
static int neom8_fetch_data(const struct device *dev)
{
	...
	LOG_INF("neom8_fetch_data call number: %d", ++call_count);
	...
	data->_buffer[0] = 0;
	data->_index = 0;
	while ((c != '\n' || data->_buffer[0] != '$') && underrun_count < TIMEOUT_COUNT_NMEA
      && data->_index < MAX_NMEA_SIZE - 1) {
		rc = read_register(dev, DATA_STREAM_ADDR, &c);

		if (rc) {
			++underrun_count;
			continue;
		} else if (c == '$' || data->_buffer[0] == '$') {
			underrun_count = 0;
...
static int neom8_send_ubx(const struct device *dev, uint8_t class, uint8_t id,
				uint8_t payload[], uint16_t length)
{
	static int call_count = 0;
	LOG_INF("neom8_send_ubx call number: %d", ++call_count);
	...
	int index = 0;
	int count = 0;
	while (index < response_size && underrun_count < TIMEOUT_COUNT_UBX) {
		rc = read_register(dev, DATA_STREAM_ADDR, &c);

		if (rc) {
			++underrun_count;
			continue;
		} else if (c == 0xB5 || response[0] == 0xB5) {
			underrun_count = 0;
			response[index++] = c;
		}
	}

	if (underrun_count < TIMEOUT_COUNT_UBX) {
		if (response[3] == 0x00) {
			rc = NACK;
...
static int neom8_cfg_nav5(const struct device *dev, enum gnss_mode g_mode,
				...
				uint16_t static_hold_max_dist, enum utc_standard utc_strd)
{
    ...
    LOG_ERR("Error %d: API didn't return ACK/NACK message.", rc);
  }

	return rc;
}

static int neom8_cfg_gnss(const struct device *dev, uint8_t msg_ver,
				uint8_t num_trk_ch_use, uint8_t num_config_blocks, ...)
{
	...
	int length = (4 + (8 * num_config_blocks));

	rc = neom8_send_ubx(dev, UBX_CLASS_CFG, UBX_CFG_GNSS, payload, length);
	if (rc == NACK) {
...
static int neom8_cfg_prt(const struct device *dev, uint8_t port_id)
{
	uint8_t payload[MAX_PAYLOAD_SIZE];
	int rc;
	...
	rc = neom8_send_ubx(dev, UBX_CLASS_CFG, UBX_CFG_PRT, payload, 20);
	if (rc == NACK) {
		LOG_ERR("Config MSG not acknowledged %s", dev->name);
	} else if (rc == ACK) {
		LOG_INF("Config MSG acknowledged %s", dev->name);
...
static int neom8_init(const struct device *dev)
{
	...
	if (!device_is_ready(cfg->uart_dev)) {
		...
		LOG_ERR("UART device %s is ready", cfg->uart_dev->name);
...
#if CONFIG_NEOM8_INIT_PRIORITY <= CONFIG_SERIAL_INIT_PRIORITY
#error CONFIG_NEOM8_INIT_PRIORITY must be greater than UART OR I2C_INIT_PRIORITY
#endif
//TODO: Add conditional code between I2C and UART2
#define NEOM8_INIT(n)                                                                    \
	static struct neom8_data neom8_data_##n;                                               \
																																												 \
	static const struct neom8_config neom8_config_##n = {                                  \
		.uart_dev = DEVICE_DT_GET(DT_CHOSEN(zephyr_gps1))                                    \
	};                                                                                     \
																																												 \
